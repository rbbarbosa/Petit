...
2. In programming languages, the names of variables and functions are generically referred to as _identifiers_, which are non-empty sequences of letters and digits, starting with a letter. Modify the code to print ``IDENTIFIER(IdName)`` whenever some identifier ``IdName`` is found.
